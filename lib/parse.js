...
parse.update_account_sharing = function (req, res) {
  ...
  https.get(options, function(response) {
    ...
    response.on('end', function() {
      var objects = JSON.parse(objects).results;
      find_objects_to_update(username, userId, objects);
      //TODO(yasumoto): Should have _much_ better error/reporting.
      res.writeHead(200);
      res.end();
    });
  }).on('error', function(e) {
